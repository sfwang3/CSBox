...
| 终端画面太散，整理报告很麻烦。 | 进入 **整理证据**，新建 **证据集**（Evidence Set），排序并编辑选中的关键画面。 | 稳定、可编辑的证据交接层。 |
...
| 我在做 API 实验。 | 导入静态 OpenAPI 3.0/3.1 模板或使用 TOML 场景，调用配置好的 URL，再导出 **API 实验**（API Evidence）。 | 脱敏 PNG、`api-evidence.md` 和 `results.json`。 |
...
**回看**（Review）可以回放已结束的 Lab，并补充、改名或删除关键画面。**证据集**引用源 Capture，可调整顺序并保存你自己的展示标题、图注和备注，不会改写原始回放。
...
Lab 导出 `evidence/<NN-title>.png`、`evidence.md`、`session.cast`、可选的 `commands.txt` 和内部生成清单。Report 导出 `report.md`、`report.docx`、`assets/*.png` 和内部生成的报告清单。API 导出 `evidence/*.png`、`api-evidence.md`、`results.json` 和内部生成清单。
