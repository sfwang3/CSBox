...
class SessionRepository:
    ...
    def list_summaries(self) -> tuple[SessionSummary, ...]:
        """List metadata and Capture counts without opening terminal recordings."""

        if not self.root.is_dir():
            return ()
        summaries: list[SessionSummary] = []
        for directory in self.root.iterdir():
            summary = self._read_summary_directory(directory)
    ...
    def read_summary(self, session_id: str) -> SessionSummary | None:
        """Read one exact lightweight summary for a Records screen refresh."""
        ...
        if (
            ...
            return None
        return self._read_summary_directory(self.root / identifier)
    ...
    def _read_summary_directory(self, directory: Path) -> SessionSummary | None:
        if not directory.is_dir() or directory.is_symlink():
            return None
        paths = SessionPaths(directory)
        if paths.cast.is_symlink():
        ...
        try:
            metadata = self._read_metadata(paths)
            if metadata.session_id != directory.name:
                return None
            metadata, _ = self._recover_stale_metadata(paths, metadata)
            capture_count = CaptureStore(paths.captures).count()
        except (OSError, UnicodeError, ValueError, SessionRepositoryError):
            return None
        return SessionSummary(paths=paths, metadata=metadata, capture_count=capture_count)
