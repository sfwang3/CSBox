...
from csbox.lab.status import LabCompletionPresenter
...
class WindowsTerminalLabLauncher:
    ...
    def start(
        ...
        *,
        ...
        if executable is None:
            ...
            store.mark_failed(
                intent,
                error_kind="windows_terminal_unavailable",
                error=error,
        ...
        try:
            ...
            store.mark_failed(
                intent,
                error_kind="windows_terminal_launch_failure",
                error=exc,
                message="Windows Terminal launch client failed",
            )
            self.last_intent_status = store.load_status(intent)
            raise LabLaunchError(
                "windows_terminal_launch_failure",
                "Windows Terminal could not be started.",
                exc,
            ) from exc
    ...
    def _find_wt(self) -> str | None:
        ...
        located = shutil.which("wt.exe") or shutil.which("wt")
...
def run_dedicated_lab_host(
    ...
    *,
    ...
    completion_presenter: Callable[..., object] | None = None,
) -> int:
    ...
    ready_session: object | None = None
    try:
        ...
        def mark_ready(paths: object) -> None:
            nonlocal ready, ready_session, ready_session_id
        ...
        if ready:
            ...
            _present_completion(
                completion_presenter,
                intent=intent,
                session_id=ready_session_id,
                status="interrupted",
                capture_count=_successful_capture_count(None, ready_session),
    ...
    if not isinstance(session_root, str):
        ...
        _present_completion(
            ...
            session_id=ready_session_id,
            status="failed",
            capture_count=_successful_capture_count(result, ready_session),
...
def _present_completion(
    ...
    *,
    ...
    if session_id is None:
        return
    selected_presenter = presenter or LabCompletionPresenter().present
    arguments: dict[str, object] = {
        "experiment_name": intent.name,
        "session_id": session_id,
        "status": status,
        "capture_count": capture_count,
    }
    if failure_reason is not None:
        arguments["failure_reason"] = failure_reason
    with suppress(BaseException):
        selected_presenter(**arguments)


def _successful_capture_count(result: object | None, session: object | None) -> int:
    statuses = getattr(result, "status_events", None)
    if isinstance(statuses, (list, tuple)):
        return sum(getattr(status, "kind", None) == "capture_succeeded" for status in statuses)
    captures_path = getattr(session, "captures", None)
    if not isinstance(captures_path, Path):
        return 0
    try:
        return len(CaptureStore(captures_path).load().captures)
    except Exception:
        return 0
...
def _host_error_kind(error: BaseException) -> str:
    if isinstance(error, ShellUnavailableError):
        return getattr(error, "kind", "shell_executable_unavailable")
