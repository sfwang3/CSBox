...
def _encode_key_event(
    ...
    terminal_nul = character == "\x00" and scan_code == 0
    if (
        virtual_key == VK_SPACE or scan_code == 0x39 or terminal_nul
    ) and control_state & _CTRL_PRESSED:
        prefix = (
...
class WindowsConsoleInputOwner:
    ...
    def close(self) -> None:
        """Stop this owner without closing the process-wide console handle."""

        self._closed = True
        self._pending_key_bytes.clear()
        self._resize_notices.clear()
