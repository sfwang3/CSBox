...
def test_host_boundary_and_capture_status_never_enter_child_or_terminal_events() -> None:
    backend = ScriptedBackend()
    output = MemoryOutput()
    events: list[TerminalEvent] = []
    presenter = HostOnlyLabPresenter(output, experiment_name="中文实验")
    proxy = TerminalProxy(
        backend,
        command=("pwsh.exe",),
...
def test_resize_warning_uses_the_same_host_only_status_channel() -> None:
    output = MemoryOutput()
    presenter = HostOnlyLabPresenter(output, experiment_name="实验")
