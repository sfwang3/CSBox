...
from csbox.lab.proxy import RawTerminalState, TerminalProxy, _encode_windows_input
...
def test_proxy_does_not_spawn_child_when_attached_windows_input_mode_setup_fails(
    ...
    backend = ScriptedBackend([b""])
    monkeypatch.setattr(proxy_module.os, "name", "nt")
    monkeypatch.setattr(proxy_module, "_windows_console_handle", lambda file_descriptor: 99)
    monkeypatch.setattr(proxy_module, "_get_windows_console_mode", lambda handle: 0x01F7)
    monkeypatch.setattr(
        proxy_module,
        "_set_windows_console_mode",
        lambda handle, mode: (_ for _ in ()).throw(OSError("SetConsoleMode failed")),
    )
    proxy = TerminalProxy(
        backend,
        command=("pwsh.exe",),
        ...
        terminal_state_factory=lambda: RawTerminalState(FileLike()),
        dispatcher=TerminalEventDispatcher([]),
        emulator=TerminalEmulator(columns=80, rows=24),
    )

    with pytest.raises(RuntimeError, match="Windows console input mode"):
        proxy.run()
