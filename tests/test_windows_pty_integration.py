...
def _read_until_interactive_prompt(
    backend: WindowsConPTYBackend,
    *,
    timeout: float = 10.0,
) -> bytes:
    """Wait for PowerShell readiness, not merely the first startup frame."""
    ...
    while time.monotonic() < deadline:
        ...
        if chunk == b"":
            break
        output.extend(chunk)
        prompt_tail = output.rsplit(b"PS ", maxsplit=1)[-1]
        if prompt_tail != output and b">" in prompt_tail:
            return bytes(output)
    pytest.fail(
        f"timed out waiting for the interactive PowerShell prompt; output={bytes(output[-500:])!r}"
    )
