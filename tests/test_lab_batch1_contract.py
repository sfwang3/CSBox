...
def test_service_transitions_starting_to_running_after_spawn_and_keeps_nonzero_exit_completed(
    tmp_path: Path,
) -> None:
    ...
    def observe_spawn() -> None:
        observed.append(repository.list_sessions()[0].metadata.status)
    ...
    assert observed[:2] == ["starting", "running"]
...
def test_only_the_active_session_owner_can_transition_lifecycle(
    ...
    assert {item.metadata.status for item in owner.list_sessions()} == {"starting"}
