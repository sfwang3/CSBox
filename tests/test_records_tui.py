...
async def _wait_for_export_result(app: CSBoxApp, records: object, pilot: object) -> None:
    await _wait_until(
        ...
        pilot,
        lambda: (
            app.screen.name == "export-result"
            and getattr(records, "_exporting", True) is False
            and (
                "导出失败"
                if getattr(app.screen, "failure_message", None) is not None
                else "导出完成"
            )
            in _screen_text(app.screen)
        ),
...
@pytest.mark.asyncio
async def test_export_result_warning_survives_missing_initial_resize_event(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repository = SessionRepository.from_cwd(tmp_path)
    paths = _create_session(
        repository,
        session_id="missing-initial-resize",
        name="缺少初始 Resize 实验",
        started_at=datetime(2026, 8, 19, 8, 0, tzinfo=UTC),
        capture_count=0,
    )
    ...
    monkeypatch.setattr(ExportResultScreen, "on_resize", lambda self, event: None)
    app = _app(tmp_path, repository)
